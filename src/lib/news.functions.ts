...
async function fetchIlbeBest(): Promise<TrendingItem[]> {
  // 실제 HTML 구조: <a href="/view/[ID]?..." class="subject [...]">[제목]
  const urls = [
    "https://www.ilbe.com/list/ilbe?listStyle=list&isAdmin=0&page=1",
    "https://www.ilbe.com/list/ilbe?page=1",
  ];
  ...
  for (const url of urls) {
    try {
      ...
      const re = /<a[^>]+href="(\/view\/\d+[^"]*)"[^>]+class="subject[^"]*"[^>]*>([^<]{2,150})/gi;
      let m: RegExpExecArray | null;
      while ((m = re.exec(html)) !== null && items.length < 10) {
        const rawPath = m[1].split("?")[0]; // 쿼리스트링 제거
        const href = `https://www.ilbe.com${rawPath}`;
        const rawTitle = m[2].replace(/\s+/g, " ").trim();
        if (!rawTitle || rawTitle.startsWith("※")) continue; // 공지 제외
