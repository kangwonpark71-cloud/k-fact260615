...
export type { QuickCheckResult, SimplifiedResult, SimplifiedClaim, NaverFactCheckItem, DaumFactCheckItem, FakeDetail, MatchedFakeCase } from "./analyses/types";
import {
  getAllActiveKeys,
  getOptionalUserId,
  validatePublicUrl,
...
function isFallbackClaim(claims: Phase1Claim[]): boolean {
  return claims.length <= 1 && claims.every(c =>
    c.claim === "본문 내 주요 주장" || c.claim.length < 10
  );
}
...
function applyAnachronismRules(claims: Phase1Claim[], bodyText = ""): Phase1Claim[] {
  ...
  const isFallback = isFallbackClaim(claims);
...
export const quickAnalyzeContent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<QuickCheckResult> => {
    // ── 병렬 실행: 문체 분석 + Naver + Daum + 공공 통계 + 최근 거짓뉴스 매칭 ──
    const [styleAnalysis, naverFactChecks, daumFactChecks, publicStats] = await Promise.all([
      Promise.resolve(buildStyleAnalysis(data.text)),
      fetchNaverFactChecks(data.text),
      fetchDaumFactChecks(data.text),
      fetchPublicDataForClaims(data.text),
    ]);

    // 최근 거짓뉴스 사례 DB 동기 매칭 (정규식 기반 — 빠름)
    const matchedFakeCases = matchRecentFakeCases(data.text);
    const recentFakeBlock  = formatRecentFakeCasesForPrompt(matchedFakeCases);

    const styleBlock      = styleAnalysisToPromptBlock(styleAnalysis);
    const naverBlock      = formatNaverBlockForPrompt(naverFactChecks);
    const daumBlock       = formatDaumBlockForPrompt(daumFactChecks);
    const publicDataBlock = formatPublicDataBlock(publicStats);
    // recentFakeBlock 은 위에서 이미 계산됨

    // ── 한국형 가짜뉴스 패턴 사전 탐지 ──
    const KOREAN_FAKE_RE =
      /\[단독\]|\[속보\]|\[긴급\]|충격적?인?\s*(반전|진실|폭로|사실)|익명의?\s*(소식통|관계자|전문가|제보자)|카카오톡\s*공유|밴드\s*공유/gi;
    const URGENCY_RE =
      /지금\s*당장|빨리\s*공유|긴급\s*전파|오늘까지|내일까지|즉시\s*확인|반드시\s*알아야/gi;
    const koreanFakeCount  = (data.text.match(KOREAN_FAKE_RE) ?? []).length;
    const urgencyCount     = (data.text.match(URGENCY_RE) ?? []).length;
    const m                = styleAnalysis.metrics;
    const citationScore    = Math.round((1 - Math.min(1, m.attributionScore * 5)) * 100);
    const patternScore     = Math.min(100, koreanFakeCount * 25);
    const urgencyScore     = Math.min(100, urgencyCount * 30);
    const crossRefCount    = naverFactChecks.length + daumFactChecks.length;

    const fakeSignals      = [...styleAnalysis.signals];
    if (koreanFakeCount > 0)
      fakeSignals.push(`한국형 가짜뉴스 패턴 ${koreanFakeCount}건 — [단독]/폭로/익명 소식통 등`);
    if (urgencyCount > 0)
      fakeSignals.push(`허위 긴박감 패턴 ${urgencyCount}건 — "지금 당장"·"빨리 공유" 등`);
    if (crossRefCount > 0)
      fakeSignals.push(`Naver·다음 팩트체크 기사 교차확인 ${crossRefCount}건`);
    ...
    const contextBlock = (data.context && data.context.length > 0)
      ? "\n[대화 맥락 - 논리 연결·모순·전제 정합성 검증]\n"
        + data.context.map(c =>
            "• " + c.speaker + ": \"" + c.text + "\""
            + (c.verdict ? " → 판정: " + c.verdict : "")
          ).join("\n")
        + "\n→ 현재 발언이 위 맥락 대비 논리 모순·인과 오류·자기 모순이 있으면 risk_flags에 반드시 명시하세요.\n"
      : "";

    const sourcesDesc = [
      naverBlock ? "네이버 팩트체크" : "",
      daumBlock  ? "다음 팩트체크"   : "",
      publicDataBlock ? "공공 통계 데이터" : "",
...
"""`;
    ...
    try {
      ...
      // ── 종합 가짜뉴스 점수 계산 (LLM risk_flags + 최근 거짓뉴스 매칭 반영) ──
      const riskPenalty = Math.min(30, llmResult.risk_flags.length * 8);
      const combined = Math.max(0, Math.min(100, Math.round(
        styleAnalysis.fakeProbability * 0.35 +
        patternScore                  * 0.18 +
        citationScore                 * 0.12 +
        urgencyScore                  * 0.10 +
        riskPenalty                   * 0.12 +
        recentCaseBonus               * 0.13 -
        crossRefCount                 * 5,
      )));
      ...
      return {
        ...llmResult,
        fake_probability: styleAnalysis.fakeProbability,
        style_signals: fakeSignals,
        naver_factchecks: naverFactChecks.length > 0 ? naverFactChecks : undefined,
        daum_factchecks:  daumFactChecks.length  > 0 ? daumFactChecks  : undefined,
        matched_fake_cases: matchedFakeCases.length > 0
          ? matchedFakeCases.map(c => ({
              id: c.id,
              category: c.category,
              title: c.title,
              period: c.period,
              verdict: c.verdict,
              briefing: c.briefing,
              debunked_by: c.debunked_by,
              confidence: c.confidence,
            }))
          : undefined,
        fake_detail: {
          style_score:     styleAnalysis.fakeProbability,
          pattern_score:   Math.min(100, patternScore + recentCaseBonus),
