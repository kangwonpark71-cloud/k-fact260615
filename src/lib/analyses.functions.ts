...
**근거 부족** — 아래 경우 포함
- 출처·수치가 불명확하여 사실/거짓 판단이 어려운 경우
- 실시간 데이터가 반드시 필요한 경우: 오늘의 주가·환율, 현재 기상, 진행 중인 사건
- 미발표 연구·비공개 자료·미래 예측이 근거의 전부인 경우

## 핵심 원칙
1. **적극 판정**: 학습 지식으로 판단 가능하면 사실 또는 반대근거우세로 단호하게 판정
2. **근거 부족 최소화**: 최후 수단으로만 사용. 전체 주장 중 근거 부족이 절반 이상이면 재검토
...
**근거 부족** (confidence 10~49): 검증 가능하나 확신이 부족하거나, 불확실 또는 실시간 데이터가 필요해 Phase 2에서 Tavily 재판정이 필요한 경우.

## 핵심 원칙
1. "반대 근거 우세"는 명백한 반증이 있을 때만 — 역사·과학·공식 기록과 명확히 상충
2. 확신 없으면 "근거 부족" (2차 검토에서 개선됨)
...
• "근거 부족"/"근거 부족": Tavily 결과로 재판정 — 증거 있으면 사실/반대근거우세로 업데이트
• "사실": Tavily로 확인·조정
...
- 근거 부족은 최후 수단 — highlights 중 근거 부족이 절반 이상이면 재검토 필요
