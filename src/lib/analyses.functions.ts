...
async function processAnalysis(
  ...
  try {
    ...
    const prompt = `다음 본문에서 검증 가능한 핵심 주장 3~7개를 추출하고, 각 주장에 대해 학습된 지식을 최대한 활용하여 명확한 팩트체크 판정을 내리세요.${sourceUrl ? `\n원본 URL: ${sourceUrl}` : ""}

판정 원칙 재확인:
• 역사·과학·법령·통계로 알 수 있는 것 → 즉시 "사실" 또는 "반대 근거 우세"로 판정
• "미확인"은 오직 실시간 데이터(현재 주가·날씨·진행 중 사건)가 반드시 필요할 때만
• 애매하다고 미확인으로 회피하지 말고, 가장 가능성 높은 판정을 선택
...
위 원칙에 따라 명확하고 단호하게 JSON으로 응답하세요.`;
...
export const quickAnalyzeContent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const quickPrompt = `다음 텍스트에서 검증 가능한 사실 주장을 추출하고 명확하게 팩트체크하세요. 학습 지식으로 판단 가능한 것은 반드시 사실/반대근거우세로 판정하고, 미확인은 실시간 데이터가 필수일 때만 사용하세요.
