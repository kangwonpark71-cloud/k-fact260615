...
const SOURCE_META: Record<SourceType, { label: string; color: string; bg: string; Icon: typeof Shield }> = {
    ...
    <div className="glass rounded-2xl overflow-hidden flex flex-col shadow-[var(--shadow-card)]">
      ...
      <div className="flex-1 overflow-y-auto" style={{ maxHeight: "calc(100vh - 280px)", minHeight: "400px" }}>
        ...
        {!isLoading && !isError && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 gap-2">
            ...
            <p className="text-xs text-muted-foreground">해당 카테고리 뉴스가 없습니다.</p>
          </div>
        )}
        ...
        {!isLoading && filtered.length > 0 && (
          <ul className="divide-y divide-border/30">
            {filtered.map((item, idx) => {
              ...
              return (
                <li key={item.id} className="group px-4 py-3.5 sm:py-3 hover:bg-surface-2/40 transition-colors">
                  <div className="flex gap-3 items-start">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <a
                        ...
                        className="text-sm sm:text-xs leading-relaxed text-foreground/90 line-clamp-2 hover:text-foreground transition-colors"
                      >
                        {item.title}
                      </a>
                    ...
                    {/* 분석하기 버튼 - 모바일은 항상 표시 */}
                    <div className="shrink-0 flex flex-col gap-1.5 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => onAnalyze(item.link)}
                        title="이 기사를 K-Fact로 분석하기"
                        className="p-2 sm:p-1.5 rounded-md bg-primary/20 hover:bg-primary/40 text-primary transition-all"
                      >
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-2 sm:p-1.5 rounded-md bg-surface-2 hover:bg-border text-muted-foreground transition-all"
                      >
      ...
      <div className="border-t border-border/40 px-4 py-2.5 sm:py-2">
        <p className="text-xs sm:text-[10px] text-muted-foreground/50 text-center">
          5분 캐시 · 연합뉴스·뉴시스·뉴스1·MBC·매일경제·정책브리핑
        </p>
      </div>
