...
function SharedAnalysisPage() {
  ...
  const items: Claim[] = Array.isArray(claimsData?.items) ? (claimsData.items as Claim[]) : [];

  const vm = getVerdictMeta(overallVerdict);
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 py-6 space-y-6">
        ...
        {items.length > 0 && (
          <div className="space-y-3">
            ...
            {items.map((claim, i) => {
              const cm = getVerdictMeta(claim.verdict);
              const CIcon = cm.icon;
              return (
                <div
                  key={i}
                  className="p-4 rounded-xl border border-border/40 bg-surface space-y-3"
                >
