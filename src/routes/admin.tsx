...
function AdminPage() {
  ...
  const chartData = chartDays === 7 ? (stats?.daily30 ?? []).slice(-7) : (stats?.daily30 ?? []);
        ...
        <div className="flex gap-1 mb-8 glass rounded-xl p-1 w-fit">
          {([
            ...
            <button
              ...
              onClick={() => {
                setTab(t);
                if (t !== "analyses") setFilterUserId(undefined);
              }}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <StatCard
                ...
                label="이번 주"
                value={statsLoading ? "—" : (stats?.week ?? 0).toLocaleString()}
                sub="최근 7일"
              ...
              <StatCard
                ...
                label="이번 달"
                value={statsLoading ? "—" : (stats?.month ?? 0).toLocaleString()}
                sub="최근 30일"
              ...
              <StatCard
                ...
                sub={`익명 ${stats?.uniqueSessions ?? 0}세션`}
                color="text-blue-400"
              />
              <StatCard
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="glass rounded-2xl p-6">
                <h2 className="text-base font-semibold mb-6">출처 신뢰도 분포</h2>
                {sourceStatsLoading ? (
                  <div className="h-24 flex items-center justify-center text-muted-foreground text-sm">
                    로딩 중…
                  </div>
                ) : sourceStats && sourceStats.totalSources > 0 ? (
                  <SourceTierChart stats={sourceStats} />
                ) : (
                  <div className="h-24 flex items-center justify-center text-muted-foreground text-sm">
                    데이터 없음
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="glass rounded-xl p-4 flex flex-wrap gap-3 items-center">
              ...
              <select
                ...
                onChange={(e) => {
                  setUserTypeFilter(e.target.value as "all" | "user" | "anon");
                  setPage(0);
                }}
                className="px-3 py-2 text-sm rounded-lg bg-background/40 border border-border outline-none focus:border-primary"
              >
                  ...
                  {usersLoading ? (
                    ...
                    filteredUsers.map((u) => (
                      ...
                      >
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p
                                className={`text-xs text-muted-foreground truncate ${!u.full_name ? "font-medium text-foreground text-sm" : ""}`}
                              >
                                {u.email}
                              </p>
                            </div>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-sm font-semibold ${u.analysis_count > 0 ? "text-primary" : "text-muted-foreground"}`}
                          >
                        ...
                        <td className="px-4 py-3 hidden md:table-cell text-xs text-muted-foreground whitespace-nowrap">
                          {u.last_analysis_at
                            ? new Date(u.last_analysis_at).toLocaleDateString("ko-KR", {
                                month: "2-digit",
                                day: "2-digit",
                              })
                            : "—"}
        ...
        {tab === "apikeys" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  활성화된 키가 AI 분석에 우선 사용됩니다. 환경 변수 GEMINI_API_KEY는 폴백으로
                  동작합니다.
                </p>
              </div>
              <button
                    ...
                    (apiKeys ?? []).map((key) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <button
                            onClick={() =>
                              toggleKeyMut.mutate({ id: key.id, is_active: !key.is_active })
                            }
                        ...
                        <td className="px-4 py-3">
                          <button
                            onClick={() => {
                              if (confirm(`"${key.name}" 키를 삭제할까요?`))
                                deleteKeyMut.mutate(key.id);
                            }}
            ...
            <div className="glass rounded-xl p-4 text-xs text-muted-foreground leading-relaxed">
              ...
              <p>
                1. DB에 등록된 <span className="text-emerald-400">활성</span> 키 중 가장 최근 등록
                순 → 2. 환경 변수 <code className="bg-surface-2 px-1 rounded">GEMINI_API_KEY</code>
              </p>
            </div>
          </div>
        )}
      ...
      {detailId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-2xl max-h-[85vh] overflow-y-auto glass rounded-2xl p-6">
            <button
              onClick={() => setDetailId(null)}
              className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-surface-2 text-muted-foreground hover:text-foreground transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            {detailLoading ? (
              ...
              <div className="space-y-5">
                <div>
                  <div className="flex items-start gap-3 mb-2">
                    ...
                    <span className="text-xs text-muted-foreground mt-1">
                      {detailData.overall_confidence}% 신뢰도
                    </span>
                ...
                {detailData.source_url && (
                  <a
                    href={detailData.source_url as string}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-xs text-primary hover:underline"
                  >
                    <ExternalLink className="w-3.5 h-3.5" />
                    {detailData.source_url as string}
                  </a>
                )}
                ...
                {Array.isArray(detailData.claims) && detailData.claims.length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-sm font-semibold">
                      주장 분석 ({(detailData.claims as unknown[]).length}개)
                    </h3>
                    {(
                      detailData.claims as Array<{
                        claim: string;
                        verdict: string;
                        confidence: number;
                        reasoning: string;
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          <span
                            className={`text-xs font-semibold ${VERDICT_TEXT[c.verdict] ?? "text-muted-foreground"}`}
                          >
                            {c.verdict}
                          </span>
...
function StatCard({
  ...
  return (
    <div className="glass rounded-2xl p-5 flex flex-col gap-3">
      <div className={`flex items-center gap-2 ${color ?? "text-muted-foreground"}`}>
        {icon}
    ...
    <div className="flex items-end gap-0.5 h-24">
      {data.map(({ hour, count }) => (
        <div key={hour} className="flex-1 flex flex-col items-center gap-1 group min-w-0">
          ...
          />
          {hour % 6 === 0 && <span className="text-[9px] text-muted-foreground">{hour}시</span>}
        </div>
      ))}
    </div>
  );
...
const TIER_LABELS: Record<string, { label: string; color: string }> = {
  authoritative: { label: "공식·학술", color: "bg-verdict-true" },
  established: { label: "검증 언론", color: "bg-blue-500" },
  standard: { label: "일반 웹", color: "bg-muted-foreground/60" },
  weak: { label: "플랫폼·커뮤니티", color: "bg-yellow-500" },
  unknown: { label: "출처 불명", color: "bg-muted/50" },
};

function SourceTierChart({
  stats,
}: {
  stats: {
  ...
  if (stats.totalSources === 0) return <p className="text-sm text-muted-foreground">데이터 없음</p>;
  const tiers = ["authoritative", "established", "standard", "weak", "unknown"] as const;
  return (
    <div className="space-y-3">
      {tiers.map((tier) => {
        const count = stats.tierCounts[tier] ?? 0;
        const pct = stats.totalSources > 0 ? Math.round((count / stats.totalSources) * 100) : 0;
        const meta = TIER_LABELS[tier];
        return (
          <div key={tier} className="flex items-center gap-3">
            <span className="text-sm w-24 shrink-0 text-muted-foreground">{meta.label}</span>
            <div className="flex-1 h-2 rounded-full bg-border overflow-hidden">
              <div
      ...
      <div className="pt-2 border-t border-border/40 text-[10px] text-muted-foreground">
        총 {stats.totalSources}개 출처 · {stats.scoredCount}건 점수 있음
        {stats.unscoredCount > 0 && ` · ${stats.unscoredCount}건 미분류`}
      </div>
    </div>
  );
}
...
function ProviderBadge({ provider }: { provider: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${meta.bg} ${meta.color}`}
    >
      {meta.label}
    </span>
  );
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-[10px] text-muted-foreground">
              키 값은 서버에만 저장되며, 이후 마지막 4자리만 표시됩니다.
            </p>
          </div>
